...
class VIWCAIO_CART_ALL_IN_ONE_Frontend_Menu_Cart {
	protected $settings;
	...
	public function create_menu_cart( $items, $args ) {
		...
		foreach ($menus as $menu){
		    if (in_array($menu->term_id, $mc_menu_display)){
		        $mc_menu_display[] = $menu->slug;
            }
        }
		...
		if ( ! $is_wcaio_menu ) {
			...
			$mc_nav_page = $this->settings->get_params( 'mc_nav_page' );
