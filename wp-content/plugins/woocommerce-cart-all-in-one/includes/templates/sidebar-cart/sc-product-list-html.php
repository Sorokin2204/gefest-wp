...
if ( ! $wc_cart || $wc_cart->is_empty() ) {
	echo sprintf( '<li class="vi-wcaio-sidebar-cart-pd-empty">%s</li>',
		apply_filters( 'vi_wcaio_get_cart_empty_text', esc_html__( 'No products in the cart.', 'woocommerce-cart-all-in-one' ) ) );
	return;
}
...
foreach ( $wc_cart->get_cart() as $cart_item_key => $cart_item ) {
	...
	if ( $product && $product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
		...
		if ( is_plugin_active( 'shopengine/shopengine.php' ) ) {
			...
			$product_name      = $product_permalink ? sprintf( '<a href="%s" class="vi-wcaio-sidebar-cart-pd-name">%s</a>', esc_url( $product_permalink ), $product->get_name() ) : sprintf( '<div class="vi-wcaio-sidebar-cart-pd-name">%s</div>', $product->get_name() );
		} else if ( ! $product_permalink ) {
			$product_name = apply_filters( 'woocommerce_cart_item_name', sprintf( '<div class="vi-wcaio-sidebar-cart-pd-name">%s</div>', $product->get_name() ), $cart_item, $cart_item_key );
		} else {
			$product_name = apply_filters( 'woocommerce_cart_item_name', sprintf( '<a href="%s" class="vi-wcaio-sidebar-cart-pd-name">%s</a>', esc_url( $product_permalink ), $product->get_name() ), $cart_item, $cart_item_key );
		}
		?>
            ...
            <div class="vi-wcaio-sidebar-cart-pd-info-wrap">
                    ...
                    <div class="vi-wcaio-sidebar-cart-pd-remove-wrap">
						...
						echo apply_filters( 'vi_wcaio_mini_cart_pd_remove',
							sprintf( '<a href="%s" class="vi-wcaio-sidebar-cart-pd-remove" aria-label="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s"><i class="%s"></i></a>',
								...
								esc_html__( 'Remove this item', 'woocommerce-cart-all-in-one' ),
								esc_attr( $product_id ),
                ...
                <div class="vi-wcaio-sidebar-cart-pd-desc">
                    ...
                    <div class="vi-wcaio-sidebar-cart-pd-price">
						<?php
						echo wp_kses( $sidebar_cart::get_sc_pd_price_html( $wc_cart, $cart_item, $cart_item_key, $product, $sc_pd_price_style ?? $settings->get_params( 'sc_pd_price_style' ) ), VIWCAIO_CART_ALL_IN_ONE_DATA::extend_post_allowed_html() );
						?>
                    </div>
                </div>
            </div>
