...
if (cart_value?.value && cart_price?.value && cart_step?.value) {
  cart_show_pallet.innerHTML = (parseFloat(cart_step?.value) * parseFloat(cart_value?.value)).toFixed(2);
  ...
  btn_plus_step.addEventListener('click', () => {
    ...
    cart_show_pallet.innerHTML = (currentVal * parseFloat(cart_value?.value)).toFixed(2);
